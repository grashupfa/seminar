...
\begin{figure}[ht!]
  \centering
\begin{tikzpicture}[level/.style={sibling distance=80mm/#1, level distance = 2cm, -latex}]
\node [circle,draw] (0) {0}
  child {node [circle,draw] (1) {1} 
        child {node [circle,draw] (3) {3} 
              child {node [circle,draw] (d1) {d1} edge from parent node[above left]{0.5}
              }
        edge from parent node[above left]{0.5}
        }
        child {node [circle,draw] (4) {4} 
               child {node [circle,draw] (d2) {d2} edge from parent node[above left]{0.5}
               }
               child {node [circle,draw] (d3) {d3} edge from parent node[above right]{0.5}
               }
  ...
  child {node [circle,draw] (2) {2}
          child {node [circle,draw] (5) {5}
              child {node [circle,draw] (d4) {d4} edge from parent node[above left]{0.5}
              }
              child {node [circle,draw] (d5) {d5} edge from parent node[above right]{0.5}
              }
        edge from parent node[above left]{0.5}
        }
        child {node [circle,draw] (6) {6}
               child {node [circle,draw] (d6) {d6} edge from parent node[above right]{0.5}
        }   
        edge from parent node[above right]{0.5}
        }
  edge from parent node[above right]{0.5}
